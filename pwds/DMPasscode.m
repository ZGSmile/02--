...
//    UINavigationController* nc = [[UINavigationController alloc] initWithRootViewController:_passcodeViewController];
    [viewController.navigationController pushViewController:_passcodeViewController animated:YES];
...
- (void)closeAndNotify:(BOOL)success {
    [_passcodeViewController.navigationController popViewControllerAnimated:success];
    ...
    if (_mode == 0) {
        if (_count == 0) {
            ...
            if ([code isEqualToString:_prevCode]) {
                NSLog(@"shezhi---密码%@",code);
                // 成功
                UIAlertView* errorAlert = [[UIAlertView alloc] initWithTitle:@"成功" message:nil delegate:nil cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
                [errorAlert show];
                ...
                UIAlertView* errorAlert = [[UIAlertView alloc] initWithTitle:@"两次密码不一致，请重新输入" message:nil delegate:nil cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
                [errorAlert show];
